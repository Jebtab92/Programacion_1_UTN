...
int eOrquesta_MostrarTodos(eOrquesta array[], int TAM) {
	...
	//SI CANTIDAD == 0 - NO HUBO Orquesta PARA MOSTRAR (ARRAY SIN ALTAS)
	if (cantidad > 0) {
		rtn = 1;
	}
...
int eOrquesta_MostrarDadosDeBaja(eOrquesta array[], int TAM) {
	...
	//SI CANTIDAD == 0 - NO HUBO Orquesta PARA MOSTRAR (ARRAY SIN BAJAS)
	if (cantidad > 0) {
		rtn = 1;
	}
...
eOrquesta eOrquesta_CargarDatos(void) {
	...
	// nombre, lugar, fecha y tipo

	Get_OnlyAlphabetStringWithSpaces("Ingrese Nombre: ", "Error, Reingrese: ", auxiliar.nombre, MAX_CHARS_CADENAS); // Pide un nombre
	Get_String("Ingrese Lugar: ", "Error, Reingrese: ", auxiliar.lugar, MAX_CHARS_CADENAS); // Pide un lugar
	auxiliar.fecha.dia = Get_IntRange("Ingrese dia: ", "Error, Reingrese: ", 1, 31);
	auxiliar.fecha.mes = Get_IntRange("Ingrese Mes: ", "Error, Reingrese: ", 1, 12);
	auxiliar.fecha.anio = Get_IntRange("Ingrese anio: ", "Error, Reingrese: ", 1900, 2021);
...
int eOrquesta_Alta(eOrquesta array[], int TAM) {
	...
	if (index != -1) {
		...
		eOrquesta_MostrarUno(auxOrquesta);
...
int eOrquesta_Baja(eOrquesta array[], int TAM) {
	...
	if(flag)
	{
		...
		idOrquesta = Get_IntPositive("Ingrese Id a dar de baja: ", "Error, Reingrese: ");
